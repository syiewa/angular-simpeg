define(['app'], function(app) {
    var url = 'admin/penghargaan'; // load aplikasi dengan nama myApp dan plugin ngRoute dan ui.bootstrap
    app.controller('editpenghargaanController', function($scope, dataService, $routeParams, $location) {
        $scope.header = "Edit Data Penghargaan";
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/penghargaan');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('newpenghargaanController', function($scope, dataService, $location) {
        $scope.header = "Tambah Data Penghargaan";
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/backend/penghargaan dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
    ...
    app.controller('listpenghargaanController', function($scope, dataService, $filter,$location) {
        $scope.header = "Data Penghargaan";
        ...
        $scope.edit = function(id) {
            $location.path('/backend/penghargaan/edit/' + id);
        };
