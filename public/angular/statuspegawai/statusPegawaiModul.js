define(['app'], function(app) {
    var url = 'admin/statuspegawai'; // load aplikasi dengan nama myApp dan plugin ngRoute dan ui.bootstrap
    app.controller('editstatuspegawaiController', function($scope, dataService, $routeParams, $location) {
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/statuspegawai');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('newstatuspegawaiController', function($scope, dataService, $location) {
        $scope.header = "Tambah Data Status Pegawai";
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/backend/statuspegawai dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
    ...
    app.controller('liststatuspegawaiController', function($scope, dataService, $filter, $location) {
        ...
        $scope.edit = function(id) {
            $location.path('/backend/statuspegawai/edit/' + id);
        };
