define(['app'], function(app) {
    var url = 'admin/lokasipelatihan'; // load aplikasi dengan nama myApp dan plugin ngRoute dan ui.bootstrap
    app.controller('editlokasipelatihanController', function($scope, dataService, $routeParams, $location) {
        $scope.header = "Edit Data Lokasi Pelatihan";
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/lokasipelatihan');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('newlokasipelatihanController', function($scope, dataService, $location) {
        $scope.header = "Tambah Data Lokasi Pelatihan";
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/backend/lokasipelatihan dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
    ...
    app.controller('listlokasipelatihanController', function($scope, dataService, $filter,$location) {
        $scope.header = "Data Lokasi Pelatihan";
        ...
        $scope.edit = function(id) {
            $location.path('/backend/lokasipelatihan/edit/' + id);
        };
