define(['app'], function(app) {
    var url = 'admin/ppk';
    app.controller('editppkController', function($scope, $routeParams, dataService, $location) {
        $scope.header = "Edit Data PPK";
        ...
        dataService.edit(url, $scope.statusId).success(function(data) {
            ...
            $scope.unitkerja = data.satuankerja;
            $scope.loading = false;
        });
    ...
    app.controller('newppkController', function($scope, dataService, $location) {
        $scope.header = "Tambah Data PPK";
        $scope.statusData = {}; //data awal bernilai array kosong;
        $scope.submitted = false; // submitted bernilai false 
        $scope.unitkerja = [];
        dataService.get('admin/dropdownsatuankerja').success(function(data) {
            $scope.satuankerja = data.dropdown;
        });
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/backend/ppk dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
    ...
    app.controller('listppkController', function($scope, $filter, dataService, $location) {
        $scope.header = "Data PPK";
        ...
        $scope.edit = function(id) {
            $location.path('/backend/ppk/edit/' + id);
        };
