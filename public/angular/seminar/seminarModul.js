define(['app'], function(app) {
    var url = 'admin/seminar';
    app.directive('ngSeminar', function() {
        return {
            restrict: 'E',
            templateUrl: 'view/seminar/list.html',
            controller: function($scope, $routeParams, dataService, $location, $filter) {
                if (!$routeParams.action) {
                    $scope.data = {};
                    $scope.header = 'Data Seminar';
                    $scope.pegawaiId = $routeParams.id;
                    dataService.get('admin/pegawai/edit/' + $routeParams.id + '/seminar').success(function(data) {
                    ...
                    $scope.edit = function(id) {
                        $location.path('/backend/seminar/edit/' + id + '/' + pegawaiId);
                    };
    ...
    app.controller('editseminarController', function($scope, $routeParams, dataService, $location) {
        ...
        $scope.loading = true;
        dataService.get('admin/seminar/create').success(function(data) {
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.idpegawai + '/seminar');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('newseminarController', function($scope, dataService, $location, $routeParams) {
        $scope.statusData = {}; //data awal bernilai array kosong;
        $scope.idpegawai = $routeParams.id
        $scope.header = "Tambah Data Seminar";
        $scope.opened = false;
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/seminar dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
                dataService.save(url, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.statusData['id_pegawai'] + '/seminar');
                            }
                        }).
                        error(function(data) {
                        });
