define(['app'], function(app) {
    var url = 'admin/keluarga';
    app.directive('ngKeluarga', function() {
        return {
            restrict: 'E',
            templateUrl: 'view/keluarga/list.html',
            controller: function($scope, $routeParams, dataService, $location, $filter) {
                if (!$routeParams.action) {
                    ...
                    dataService.get('admin/pegawai/edit/' + $routeParams.id + '/keluarga').success(function(data) {
                    ...
                    $scope.edit = function(id) {
                        $location.path('/backend/keluarga/edit/' + id + '/' + pegawaiId);
                    };
    ...
    app.controller('editkeluargaController', function($scope, $routeParams, dataService, $location) {
        $scope.header = "Edit Data Keluarga";
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.idpegawai + '/keluarga');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('newkeluargaController', function($scope, dataService, $location, $routeParams) {
        $scope.idpegawai = $routeParams.id
        $scope.header = "Tambah Data Keluarga";
        $scope.opened = false;
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/keluarga dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
                dataService.save(url, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.statusData['id_pegawai'] + '/keluarga');
                            }
                        }).
                        error(function(data) {
                        });
    ...
    app.controller('listkeluargaController', function($scope, $filter, dataService, $location) {
        ...
        $scope.edit = function(id) {
            $location.path('/backend/keluarga/edit/' + id);
        };
        $scope.sort = function(field) {
            $scope.data = $filter('orderBy')($scope.data, field, $scope.sort.order);
            $scope.sort.field = field;
