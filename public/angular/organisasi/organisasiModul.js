define(['app'], function(app) {
    var url = 'admin/organisasi';
    app.directive('ngOrganisasi', function() {
        return {
            restrict: 'E',
            templateUrl: 'view/organisasi/list.html',
            controller: function($scope, $routeParams, dataService, $location, $filter) {
                if (!$routeParams.action) {
                    $scope.data = {};
                    $scope.header = 'Data Organisasi';
                    $scope.pegawaiId = $routeParams.id;
                    dataService.get('admin/pegawai/edit/' + $routeParams.id + '/organisasi').success(function(data) {
                    ...
                    $scope.edit = function(id) {
                        $location.path('/backend/organisasi/edit/' + id + '/' + pegawaiId);
                    };
    ...
    app.controller('editorganisasiController', function($scope, $routeParams, dataService, $location) {
        ...
        $scope.loading = true;
        dataService.get('admin/organisasi/create').success(function(data) {
            ...
            if (isValid) {
                dataService.update(url, $scope.statusId, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.idpegawai + '/organisasi');
                            }
                        }).
                        error(function(data) {
                            console.log(data);
    ...
    app.controller('neworganisasiController', function($scope, dataService, $location, $routeParams) {
        $scope.statusData = {}; //data awal bernilai array kosong;
        $scope.idpegawai = $routeParams.id
        $scope.header = "Tambah Data Organisasi";
        $scope.opened = false;
        ...
        $scope.processForm = function(isValid) { // fungsi dimana saat proses form terjadi
            // jika valid maka akan mengirimkan data ke url admin/organisasi dengan $scope.statusData sebagai datanya , dan jika sukses post data maka akan kembali ke base url.
            if (isValid) {
                dataService.save(url, $scope.statusData).
                        success(function(data) {
                            if (data.success) {
                                $location.path('/backend/pegawai/edit/' + $scope.statusData['id_pegawai'] + '/organisasi');
                            }
                        }).
                        error(function(data) {
                        });
